...
// const NATIONALITIES = [
//   'Afghanistan', 'Algeria', 'Bahrain', 'Canada', 'China', 'Egypt', 'France',
//   'Germany', 'India', 'Indonesia', 'Iran', 'Iraq', 'Italy', 'Japan',
//   'Jordan', 'Kuwait', 'Lebanon', 'Malaysia', 'Morocco', 'Netherlands',
...
// const OCCUPATIONS = ['Private', 'Government', 'Self Employed', 'Retired', 'Student', 'Housewife'];
// const PROPERTY_TYPES = ['Ready', 'Off-plan', 'Commercial'];
// const PROPERTY_SUBTYPES = {
//   Ready: ['Apartment', 'Villa', 'Townhouse', 'Penthouse'],
//   'Off-plan': ['Apartment', 'Villa', 'Townhouse'],
...
//         <div className="relative p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
//           <div>
...
// const LeadCaptureModal = ({ isOpen, onClose, calculatorData }) => {
...
//     occupation: '',
//     employer: '',
//     monthlySalary: calculatorData.monthlyIncome || 25000,
//     // Property Info
//     propertyType: 'Ready',
//     propertySubtype: 'Apartment',
//     propertyValue: calculatorData.propertyValue || 1500000,
//     downPaymentAmount: calculatorData.downpayment || 300000,
//     propertyArea: '',
//     isOffPlan: false,
//     foundProperty: 'no',
//     location: '',
...
//     setFormData({ ...formData, selectedCountry: country, mobileNumber: '' });
//   };

//   const calculateLoanAmount = () => {
//     return Math.max(0, (formData.propertyValue || 0) - (formData.downPaymentAmount || 0));
//   };
...
//       return;
//     }
//     if (formData.mobileNumber.length !== formData.selectedCountry.maxLength) {
//       toast.error(`Please enter a valid ${formData.selectedCountry.maxLength}-digit phone number`);
//       return;
//     }
...
//           mobileNumber: `${formData.selectedCountry.dialCode}${formData.mobileNumber}`,
...
//           propertyAddress: {
//             area: formData.propertyArea || formData.location || '',
//             city: 'Dubai'
//           },
//           isOffPlan: formData.isOffPlan
//         },
//         notesToXoto: `
// Lead from Mortgage Calculator
// Monthly Income: ${formData.monthlySalary}
// Property Value: ${formData.propertyValue}
// Down Payment: ${formData.downPaymentAmount}
// Requested Loan: ${loanAmount}
// ${formData.notesToXoto ? `Notes: ${formData.notesToXoto}` : ''}
// `
//       };
...
//         toast.success('Our mortgage advisor will contact you within 24 hours!', { duration: 5000 });
//       } else {
//         toast.error(response?.message || 'Something went wrong. Please try again.', { id: toastId });
//       }
//     } catch (error) {
//       console.error('Lead submit error:', error);
//       toast.error(error.response?.data?.message || 'Network error. Please try again.', { id: toastId });
//     } finally {
//       setLoading(false);
//     }
//   };
...
//   return (
//     <ModalWrapper isOpen={isOpen} onClose={onClose} title="Get Pre-Approved" subtitle="Fill in your details to get started">
//       <form onSubmit={handleSubmit} className="space-y-5">
//         {/* Section 1: Personal Information */}
//         <div className="bg-purple-50 rounded-xl p-4">
//           <h3 className="text-md font-bold text-purple-800 mb-3 flex items-center gap-2"><FaUser /> Personal Information</h3>
//           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Full Name *</label>
//               <input type="text" value={formData.fullName} onChange={(e) => handleChange('fullName', e.target.value)} placeholder="Ahmed Al Mansouri" className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none" required />
//             </div>
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Email *</label>
//               <input type="email" value={formData.email} onChange={(e) => handleChange('email', e.target.value)} placeholder="ahmed@example.com" className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none" required />
//             </div>
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Phone Number *</label>
//               <div className="flex border border-slate-200 rounded-xl bg-white focus-within:ring-2 focus-within:ring-purple-500 overflow-hidden">
//                 <div className="flex items-center pl-3 pr-2 bg-slate-50 border-r border-slate-200">
//                   <img src={`https://flagcdn.com/w20/${formData.selectedCountry.code.toLowerCase()}.png`} alt={formData.selectedCountry.code} className="w-5 h-auto mr-1.5 rounded" />
//                   <select value={formData.selectedCountry.code} onChange={(e) => handleCountryChange(e.target.value)} className="bg-transparent font-semibold text-slate-700 outline-none text-sm cursor-pointer">
//                     {COUNTRIES.map(c => <option key={c.code} value={c.code}>{c.dialCode}</option>)}
//                   </select>
//                 </div>
//                 <input type="tel" value={formData.mobileNumber} onChange={handlePhoneChange} placeholder="50 XXX XXXX" className="w-full p-3 bg-white outline-none" required />
//               </div>
//             </div>
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Date of Birth *</label>
//               <input type="date" value={formData.dateOfBirth} onChange={(e) => handleChange('dateOfBirth', e.target.value)} className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none" required />
//             </div>
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Nationality *</label>
//               <select value={formData.nationality} onChange={(e) => handleChange('nationality', e.target.value)} className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none" required>
...
//               <select value={formData.maritalStatus} onChange={(e) => handleChange('maritalStatus', e.target.value)} className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none">
//                 {MARITAL_STATUS.map(m => <option key={m} value={m}>{m}</option>)}
//               </select>
//             </div>
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Number of Dependents</label>
//               <input type="number" min="0" max="20" value={formData.numberOfDependents} onChange={(e) => handleChange('numberOfDependents', Number(e.target.value))} className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none" />
//             </div>
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Occupation</label>
//               <select value={formData.occupation} onChange={(e) => handleChange('occupation', e.target.value)} className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none">
//                 <option value="">Select Occupation</option>
//                 {OCCUPATIONS.map(o => <option key={o} value={o}>{o}</option>)}
//               </select>
//             </div>
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Employer</label>
//               <input type="text" value={formData.employer} onChange={(e) => handleChange('employer', e.target.value)} placeholder="Company name" className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none" />
//             </div>
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Monthly Salary (AED)</label>
//               <input type="number" value={formData.monthlySalary} onChange={(e) => handleChange('monthlySalary', Number(e.target.value))} className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none" />
//             </div>
//           </div>
//         </div>
...
//           <h3 className="text-md font-bold text-blue-800 mb-3 flex items-center gap-2"><FaHome /> Property Details</h3>
//           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Property Type *</label>
//               <select value={formData.propertyType} onChange={(e) => handleChange('propertyType', e.target.value)} className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none" required>
//                 {PROPERTY_TYPES.map(pt => <option key={pt} value={pt}>{pt}</option>)}
//               </select>
//             </div>
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Property Subtype</label>
//               <select value={formData.propertySubtype} onChange={(e) => handleChange('propertySubtype', e.target.value)} className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none">
//                 {PROPERTY_SUBTYPES[formData.propertyType]?.map(ps => <option key={ps} value={ps}>{ps}</option>)}
//               </select>
//             </div>
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Property Value (AED) *</label>
//               <input type="number" value={formData.propertyValue} onChange={(e) => handleChange('propertyValue', Number(e.target.value))} className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none" required />
//             </div>
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Down Payment (AED)</label>
//               <div className="flex items-center gap-2">
//                 <input type="number" value={formData.downPaymentAmount} onChange={(e) => handleChange('downPaymentAmount', Number(e.target.value))} className="flex-1 p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none" />
//                 <span className="text-sm font-semibold text-purple-600 whitespace-nowrap">
//                   {formData.propertyValue > 0 ? ((formData.downPaymentAmount / formData.propertyValue) * 100).toFixed(0) : 0}%
//                 </span>
//               </div>
//             </div>
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Property Area / Location</label>
//               <input type="text" value={formData.propertyArea} onChange={(e) => handleChange('propertyArea', e.target.value)} placeholder="e.g., Downtown Dubai" className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none" />
//             </div>
//             <div className="flex items-center gap-4">
//               <label className="flex items-center gap-2 cursor-pointer">
//                 <input type="checkbox" checked={formData.isOffPlan} onChange={(e) => handleChange('isOffPlan', e.target.checked)} className="w-5 h-5 accent-purple-600" />
//                 <span className="text-sm font-medium">Off-plan Property</span>
//               </label>
//             </div>
//           </div>
          
//           {/* Loan Amount Summary Box */}
//           <div className="mt-4 bg-purple-100 rounded-xl p-3">
//             <div className="flex justify-between items-center">
//               <span className="text-sm font-semibold text-purple-700">Estimated Loan Amount:</span>
//               <span className="text-xl font-bold text-purple-800">{formatCurrency(calculateLoanAmount())}</span>
//             </div>
//             <div className="flex justify-between text-xs text-purple-600 mt-1">
//               <span>Property: {formatCurrency(formData.propertyValue)}</span>
//               <span>Down Payment: {formatCurrency(formData.downPaymentAmount)}</span>
//             </div>
//           </div>
//         </div>
...
//           <h3 className="text-md font-bold text-slate-700 mb-3 flex items-center gap-2"><FaFileAlt /> Additional Information</h3>
//           <div className="grid grid-cols-1 gap-4">
//             <div>
//               <label className="block text-xs font-semibold text-slate-600 mb-1">Have you found a property?</label>
//               <div className="flex gap-4">
//                 <label className="flex items-center gap-2 cursor-pointer">
//                   <input type="radio" name="foundProperty" checked={formData.foundProperty === 'yes'} onChange={() => handleChange('foundProperty', 'yes')} className="accent-purple-600" />
//                   <span>Yes</span>
//                 </label>
//                 <label className="flex items-center gap-2 cursor-pointer">
//                   <input type="radio" name="foundProperty" checked={formData.foundProperty === 'no'} onChange={() => handleChange('foundProperty', 'no')} className="accent-purple-600" />
//                   <span>No</span>
//                 </label>
//               </div>
//             </div>
...
//               <div>
//                 <label className="block text-xs font-semibold text-slate-600 mb-1">Property Location *</label>
//                 <select value={formData.location} onChange={(e) => handleChange('location', e.target.value)} className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none" required>
...
//               <textarea rows="3" value={formData.notesToXoto} onChange={(e) => handleChange('notesToXoto', e.target.value)} placeholder="Any special requirements or preferences..." className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none resize-none" />
//             </div>
//           </div>
//         </div>
...
//             <div>
//               <p className="text-xs text-purple-600">Monthly Income</p>
//               <p className="font-bold text-purple-800">{formatCurrency(formData.monthlySalary)}</p>
//             </div>
//             <div>
//               <p className="text-xs text-purple-600">Property Value</p>
//               <p className="font-bold text-purple-800">{formatCurrency(formData.propertyValue)}</p>
//             </div>
//             <div>
//               <p className="text-xs text-purple-600">Down Payment</p>
//               <p className="font-bold text-purple-800">{formatCurrency(formData.downPaymentAmount)}</p>
//             </div>
//             <div>
//               <p className="text-xs text-purple-600">Est. EMI</p>
//               <p className="font-bold text-purple-800">{formatCurrency(calculateEMI(calculateLoanAmount(), calculatorData.rate || 4.15, calculatorData.loanDuration || 25))}</p>
//             </div>
//           </div>
//         </div>
...
//           <input required type="checkbox" checked={formData.consent} onChange={(e) => handleChange('consent', e.target.checked)} className="mt-1 accent-purple-600" />
//           <span className="text-xs text-slate-500 leading-relaxed">I agree to receive newsletters and marketing communications. I accept the Terms of Service and Privacy Policy.</span>
//         </label>

//         <button type="submit" disabled={loading} className="w-full bg-purple-600 text-white py-4 rounded-xl font-semibold text-lg hover:bg-purple-700 transition disabled:opacity-70 flex justify-center items-center gap-2">
...
//             <span>Downpayment (15%)</span><span className="font-semibold">{formatCurrency(downpayment)}</span>
//           </div>
//           <div className="flex justify-between text-slate-600 font-medium">
//             <span>Transaction costs (5.5%)</span><span className="font-semibold">{formatCurrency(transactionCosts)}</span>
//           </div>
//           <div className="pt-3 border-t border-slate-200 flex justify-between text-lg font-bold text-purple-900">
//             <span>Total Upfront Cost</span><span>{formatCurrency(upfrontCost)}</span>
//           </div>
//         </div>
//         <div className="bg-purple-50 border border-purple-100 rounded-2xl p-5 text-center">
//           <p className="text-xs font-semibold text-purple-400 mb-1">Maximum Home Price</p>
//           <p className="text-3xl font-bold text-purple-700">{formatCurrency(youCanBorrow)}</p>
//         </div>
...
//         </div>
//         <button onClick={onClose} className="w-full bg-purple-600 text-white py-4 rounded-xl font-semibold hover:bg-purple-700 transition">Got it</button>
//       </div>
//     </ModalWrapper>
//   );
// };
...
//   const [modals, setModals] = useState({ summary: false, preapproval: false });
...
//   return (
//     <div className="min-h-screen bg-slate-50 p-4 md:p-8" style={{ fontFamily: '"DM Sans", sans-serif' }}>
//       <Toaster position="top-center" reverseOrder={false} />
//       <div className="max-w-6xl mx-auto">
//         <div className="text-center mb-10">
//           <h1 className="text-4xl md:text-5xl font-bold text-slate-900 mb-3">Mortgage Calculator</h1>
//           <p className="text-slate-500 text-lg">Smart property financing for the UAE market</p>
//         </div>

//         <div className="grid lg:grid-cols-12 gap-8">
//           {/* Left Side Info */}
//           <div className="lg:col-span-4">
//             <h1 className="text-5xl font-bold mt-10">Discover Your True Buying Power</h1>
//             <p className="text-slate-700 mt-5">Smart property financing for the UAE market.</p>
//           </div>

//           {/* Calculator Form */}
//           <div className="lg:col-span-4 bg-white p-6 rounded-2xl shadow-lg">
//             <div className="flex p-1.5 bg-slate-100/80 rounded-xl mb-8">
//               <button onClick={() => setActiveTab('affordability')} className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-semibold transition-all ${activeTab === 'affordability' ? 'bg-white text-[#5C039B] shadow-sm' : 'text-slate-500'}`}>
//                 <FaCalculator /> Buying Power
//               </button>
//               <button onClick={() => setActiveTab('mortgage')} className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-semibold transition-all ${activeTab === 'mortgage' ? 'bg-white text-[#5C039B] shadow-sm' : 'text-slate-500'}`}>
//                 <FaMoneyBillWave /> EMI Planner
//               </button>
//             </div>
...
//                   <div><label className="text-xs font-semibold text-slate-500">Residency</label><select value={residency} onChange={(e) => setResidency(e.target.value)} className="w-full p-4 bg-slate-50 border rounded-xl"><option>UAE Resident</option><option>UAE National</option><option>Non-Resident</option></select></div>
//                   <div><label className="text-xs font-semibold text-slate-500">Employment</label><select value={employment} onChange={(e) => setEmployment(e.target.value)} className="w-full p-4 bg-slate-50 border rounded-xl"><option value="">Select</option><option value="salaried">Salaried</option><option value="self_employed">Self-Employed</option></select></div>
//                 </div>
//                 <div className="grid grid-cols-2 gap-4">
//                   <div><label className="text-xs font-semibold text-slate-500">Monthly Income (AED)</label><input type="number" value={monthlyIncome} onChange={(e) => setMonthlyIncome(e.target.value)} className="w-full p-4 bg-slate-50 border rounded-xl" /></div>
//                   <div><label className="text-xs font-semibold text-slate-500">Monthly Debts (AED)</label><input type="number" value={monthlyDebt} onChange={(e) => setMonthlyDebt(e.target.value)} className="w-full p-4 bg-slate-50 border rounded-xl" /></div>
//                 </div>
...
//             {activeTab === 'mortgage' && (
//               <div className="space-y-6">
//                 <div><label className="text-xs font-semibold text-slate-500">Select Rate Type</label><div className="grid grid-cols-3 gap-2 mt-2">{PRODUCTS.map(p => (<button key={p.id} onClick={() => setSelectedProduct(p)} className={`p-3 text-xs font-semibold border rounded-xl ${selectedProduct.id === p.id ? 'border-purple-600 bg-purple-50 text-purple-700' : 'border-slate-200'}`}>{p.label}</button>))}</div></div>
//                 <div className="grid grid-cols-2 gap-4"><div><label className="text-xs font-semibold text-slate-500">Property Value (AED)</label><input type="number" value={propertyValue} onChange={(e) => setPropertyValue(e.target.value)} className="w-full p-4 bg-slate-50 border rounded-xl" /></div><div><label className="text-xs font-semibold text-slate-500">Downpayment</label><input type="number" value={downpayment} onChange={(e) => setDownpayment(e.target.value)} className="w-full p-4 bg-slate-50 border rounded-xl" /></div></div>
//                 <div><label className="text-xs font-semibold text-slate-500">Loan Duration: {loanDuration} years</label><input type="range" min={1} max={25} value={loanDuration} onChange={(e) => setLoanDuration(Number(e.target.value))} className="w-full accent-purple-600" /></div>
//               </div>
//             )}
//           </div>

//           {/* Results Card with Button */}
//           <div className="lg:col-span-4">
//             <div className="bg-gradient-to-br from-purple-900 to-indigo-900 rounded-2xl p-8 text-white shadow-2xl flex flex-col justify-between h-full">
//               <div>
//                 <p className="text-purple-300 font-semibold text-xs mb-6">{activeTab === 'affordability' ? 'Your Buying Power' : 'Cost Breakdown'}</p>
//                 {activeTab === 'affordability' ? (
//                   isEligible ? (
//                     <div>
//                       <p className="text-purple-200 text-sm mb-1">Max Property Price</p>
//                       <h2 className="text-4xl md:text-5xl font-bold mb-8">{formatCurrency(affordability)}</h2>
//                       <div className="border-t border-purple-800/50 pt-6 flex justify-between items-center">
//                         <div><p className="text-purple-300 text-xs">Est. Monthly EMI</p><p className="text-xl font-bold">{formatCurrency(monthlyPayment)}</p></div>
//                         <button onClick={() => openModal('summary')} className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-xl text-sm font-semibold flex items-center gap-2">Breakdown <FaArrowRight /></button>
//                       </div>
//                     </div>
//                   ) : (
//                     <div className="bg-white/10 border border-white/20 rounded-2xl p-6"><p className="font-bold mb-2">Eligibility Issue</p><p className="text-sm">Minimum salary of AED {MIN_SALARY.toLocaleString()} required</p></div>
//                   )
//                 ) : (
//                   <div>
//                     <p className="text-purple-200 text-sm mb-1">Monthly Installment</p>
//                     <h2 className="text-4xl md:text-5xl font-bold mb-8">{formatCurrency(monthlyEMI)}</h2>
//                     <div className="border-t border-purple-800/50 pt-6 grid grid-cols-2 gap-4">
//                       <div><p className="text-purple-300 text-xs">Total Loan</p><p className="text-xl font-bold">{formatCurrency(loanAmount)}</p></div>
//                       <div><p className="text-purple-300 text-xs">Interest Rate</p><p className="text-xl font-bold">{selectedProduct.rate}%</p></div>
//                     </div>
//                   </div>
//                 )}
//               </div>
//               <button onClick={() => openModal('preapproval')} className="mt-8 w-full bg-white text-purple-900 py-4 rounded-xl font-semibold text-lg hover:bg-gray-100 transition flex justify-center items-center gap-2">Get Pre-Approved <FaArrowRight /></button>
//             </div>
//           </div>
//         </div>
//       </div>
