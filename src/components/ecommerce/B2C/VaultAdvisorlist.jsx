...
import {
  ...
  Typography, message, Tooltip, Progress, Modal, Popconfirm,
} from "antd";
import {
...
const GN = "#22C55E";
const RD = "#EF4444";
const OR = "#F97316"; // orange → Suspend
// ──────────────────────────────────────────────────────────────────────────────
...
const VaultAdvisorlist = () => {
  ...
  const fetchAdvisors = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiService.get("/vault/advisor/all", {
  ...
  const handleSuspend = async () => {
    ...
    try {
      await apiService.post(`/vault/advisor/suspend/${suspendTarget.id}`, {
  ...
  const handleActivate = async (record) => {
    ...
    try {
      ...
      message.success(`Advisor "${fullName(record)}" activated`);
      fetchAdvisors();
    } catch (err) {
  ...
  const handleDelete = async (record) => {
    ...
    try {
      ...
      message.success(`Advisor "${fullName(record)}" deleted`);
      fetchAdvisors();
    } catch (err) {
          ...
          <Space size={4}>
            ...
            <Tooltip title="View Details">
              <Button
                ...
                size="small"
                icon={<EyeOutlined />}
                onClick={() => navigate(`../advisor/${id}`)}
                style={{ color: P }}
              />
            </Tooltip>

            {/* ── Suspend  (active & not already suspended) ── */}
            {active && !suspended && (
            ...
            {/* ── Activate  (inactive or suspended) ── */}
            {(!active || suspended) && (
