  ...
  const handleSubmit = async (values) => {
    ...
    try {
      const payload = {
        // ─── customerInfo ─────────────────────────────────────────────
        customerInfo: {
          fullName:           values.fullName,                         // ✅ REQUIRED — string
          preferredName:      values.preferredName    || null,         // 🟡 OPTIONAL — string | null
          email:              values.email,                            // ✅ REQUIRED — string
          mobileNumber:       values.mobileNumber     || null,         // ✅ REQUIRED — full number with country code (react-phone-input-2 returns e.g. "971551234567")
          alternativePhone:   values.alternativePhone || null,         // 🟡 OPTIONAL — string | null
          whatsappNumber:     values.whatsappNumber   || null,         // 🟡 OPTIONAL — string | null
          dateOfBirth:        values.dateOfBirth      ? values.dateOfBirth.toISOString() : null,   // ✅ REQUIRED — ISO date string
          nationality:        values.nationality,                      // ✅ REQUIRED — SELECT: NATIONALITIES[]
          maritalStatus:      values.maritalStatus,                    // ✅ REQUIRED — SELECT: ["Single","Married","Divorced","Widowed"]
          numberOfDependents: values.numberOfDependents  ?? 0,        // 🟡 OPTIONAL — number (default 0)
          occupation:         values.occupation          || null,      // 🟡 OPTIONAL — string | null
          employer:           values.employer             || null,     // 🟡 OPTIONAL — string | null
          monthlySalary:      values.monthlySalary        || null,     // 🔥 STRONGLY RECOMMENDED — number | null
        },

        // ─── propertyDetails ──────────────────────────────────────────
        propertyDetails: {
          propertyType:    values.propertyType,                        // ✅ REQUIRED — SELECT: ["Ready","Off-plan","Commercial"]
          propertySubtype: values.propertySubtype || null,             // 🟡 OPTIONAL — SELECT: ["Apartment","Villa","Townhouse","Penthouse"]
          propertyValue:   values.propertyValue,                      // ✅ REQUIRED — number (AED)
          downPaymentAmount:  values.downPaymentAmount   || null,      // 🟡 OPTIONAL — number | null
          loanAmountRequired: values.loanAmountRequired  || null,      // 🟡 OPTIONAL — number | null (auto-calc also fine)
          propertyAddress: {
            building: values.building || null,                         // 🟡 OPTIONAL — string | null
            area:     values.area     || null,                         // 🟡 OPTIONAL — SELECT from UAE_AREAS[city] | null
            city:     values.city     || 'Dubai',                      // 🟡 OPTIONAL — SELECT: Object.keys(UAE_AREAS) (default "Dubai")
          },
          propertyAgeYears: values.propertyAgeYears || null,           // 🟡 OPTIONAL — number | null
          isOffPlan:        values.isOffPlan         || false,          // 🟡 OPTIONAL — BOOLEAN toggle (default false)
          completionDate:   values.isOffPlan && values.completionDate   // 🟡 OPTIONAL — required only if isOffPlan=true
        ...
        // ─── loanRequirements ─────────────────────────────────────────
        loanRequirements: {
          preferredTenureYears:        values.preferredTenureYears       || 25,   // number (default 25)
          preferredInterestRateType:   values.preferredInterestRateType  || 'Fixed', // SELECT: ["Fixed","Variable"]
          preferredBanks:              preferredBanks,                    // array of strings from BANK_LIST
          feeFinancingPreference:      values.feeFinancingPreference      ?? true,  // BOOLEAN
          lifeInsurancePreference:     values.lifeInsurancePreference     ?? true,  // BOOLEAN
          propertyInsurancePreference: values.propertyInsurancePreference ?? true,  // BOOLEAN
          specialRequirements:         values.specialRequirements         || null,  // string | null
        },

        // ─── top-level ────────────────────────────────────────────────
        referralType: values.referralType || 'Referral Only',           // 🟡 OPTIONAL — SELECT: ["Referral Only","Referral + Docs"] (default exists)
        notesToXoto:  values.notesToXoto  || null,                       // 🟡 OPTIONAL — string | null
      };
...
>
          ...
          <div style={{ background: '#fff', borderRadius: 20, border: '1px solid #f0e6ff', padding: '24px 28px', marginBottom: 20, boxShadow: '0 2px 12px rgba(92,3,155,0.05)' }}>
            ...
            <Row gutter={[16, 4]}>
              ...
              {/* mobileNumber — ✅ REQUIRED — react-phone-input-2 returns full number "971551234567" */}
              <Col xs={24} sm={12} md={8}>
              ...
              {/* nationality — ✅ REQUIRED — SELECT from NATIONALITIES[] */}
              <Col xs={24} sm={12} md={8}>
              ...
              {/* maritalStatus — ✅ REQUIRED — SELECT: ["Single","Married","Divorced","Widowed"] */}
              <Col xs={24} sm={12} md={8}>
              ...
              {/* numberOfDependents — 🟡 OPTIONAL — number default 0 */}
              <Col xs={24} sm={12} md={8}>
          ...
          <div style={{ background: '#fff', borderRadius: 20, border: '1px solid #f0e6ff', padding: '24px 28px', marginBottom: 20, boxShadow: '0 2px 12px rgba(92,3,155,0.05)' }}>
            ...
            <Row gutter={[16, 4]}>
              ...
              {/* propertyType — ✅ REQUIRED — SELECT: ["Ready","Off-plan","Commercial"] */}
              <Col xs={24} sm={12} md={8}>
              ...
              {/* propertySubtype — 🟡 OPTIONAL — SELECT: ["Apartment","Villa","Townhouse","Penthouse"] */}
              <Col xs={24} sm={12} md={8}>
              ...
              {/* propertyValue — ✅ REQUIRED — number (AED) */}
              <Col xs={24} sm={12} md={8}>
              ...
              {/* downPaymentAmount — 🟡 OPTIONAL — number | null */}
              <Col xs={24} sm={12} md={8}>
              ...
              {/* loanAmountRequired — 🟡 OPTIONAL — number | null */}
              <Col xs={24} sm={12} md={8}>
              ...
              {/* propertyAddress.city — 🟡 OPTIONAL — SELECT: Object.keys(UAE_AREAS) (default "Dubai") */}
              <Col xs={24} sm={12} md={8}>
                <Form.Item name="city" label={<Label opt>City</Label>}>
                  <Select onChange={(val) => { setSelectedCity(val); form.setFieldValue('area', undefined); }} style={{ borderRadius: 8 }}>
                    {Object.keys(UAE_AREAS).map(city => <Option key={city} value={city}>{city}</Option>)}
                  </Select>
                </Form.Item>
              </Col>

              {/* propertyAddress.area — 🟡 OPTIONAL — SELECT from UAE_AREAS[selectedCity] */}
              <Col xs={24} sm={12} md={8}>
              ...
              {/* propertyAddress.building — 🟡 OPTIONAL */}
              <Col xs={24} sm={12} md={8}>
              ...
              {/* propertyAgeYears — 🟡 OPTIONAL — number | null */}
              <Col xs={24} sm={12} md={8}>
              ...
              {/* isOffPlan — 🟡 OPTIONAL — BOOLEAN toggle (default false) */}
              <Col xs={24} sm={12} md={8}>
              ...
              {/* completionDate — 🟡 OPTIONAL — required only if isOffPlan = true */}
              <Col xs={24} sm={12} md={8}>
                <Form.Item
                  name="completionDate"
          ...
          <div style={{ background: '#fff', borderRadius: 20, border: '1px solid #f0e6ff', padding: '24px 28px', marginBottom: 20, boxShadow: '0 2px 12px rgba(92,3,155,0.05)' }}>
            ...
            <Row gutter={[16, 4]}>
              ...
              {/* preferredTenureYears — number default 25 */}
              <Col xs={24} sm={12} md={8}>
              ...
              {/* preferredInterestRateType — SELECT: ["Fixed","Variable"] */}
              <Col xs={24} sm={12} md={8}>
              ...
              {/* preferredBanks — array of strings from BANK_LIST */}
              <Col xs={24}>
              ...
              {/* Insurance & fee preferences — BOOLEAN toggles */}
              <Col xs={24}>
          ...
          <div style={{ background: '#fff', borderRadius: 20, border: '1px solid #f0e6ff', padding: '24px 28px', marginBottom: 24, boxShadow: '0 2px 12px rgba(92,3,155,0.05)' }}>
            ...
            <Row gutter={[16, 4]}>
              ...
              {/* referralType — 🟡 OPTIONAL — SELECT: ["Referral Only","Referral + Docs"] (default "Referral Only") */}
              <Col xs={24} md={12}>
...
>
  ...
  >
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      <span style={{ fontSize: 11, color: "#94a3b8" }}>
        I am only referring — Xoto will handle documents
      </span>
    </div>
  </Option>
  ...
  >
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      <span style={{ fontSize: 11, color: "#94a3b8" }}>
        I will collect and submit documents too
      </span>
    </div>
  </Option>
</Select>
              ...
              {/* notesToXoto — 🟡 OPTIONAL — string | null */}
              <Col xs={24} md={12}>
