...
// Auto-open wizard with prefilled data when navigated from property page
useEffect(() => {
  const incoming = location.state?.prefill;
  const shouldAutoOpen = location.state?.autoOpenWizard;

  if (incoming && shouldAutoOpen) {
    // Prefill the wizard form
    setForm({
      title:    incoming.title    || "",
      tone:     incoming.tone     || "professional",
      language: incoming.language || "English",
      currency: incoming.currency || "AED",
      areaUnit: incoming.areaUnit || "sqft",
    });
    setSections({
      cover:    incoming.sections?.cover    ?? true,
      desc:     incoming.sections?.desc     ?? true,
      dev:      incoming.sections?.dev      ?? true,
      prices:   incoming.sections?.prices   ?? true,
      payment:  incoming.sections?.payment  ?? true,
      location: incoming.sections?.location ?? true,
    });
