...
export default function SecondaryPlans() {
  ...
  const fetchProjects = async (pageNo = 1, append = false) => {
    try {
      ...
      // ✅ Updated API endpoint: show only agent's own properties
      const res = await apiService.get(
        `properties/agency/properties/own?page=${pageNo}&limit=20`
      );
  ...
  useEffect(() => {
    ...
    const result = projects.filter((p) => {
      ...
      const matchUnitType =
        ...
        selectedUnitTypes.some((ut) =>
          pType.includes(ut.toLowerCase().replace("s", ""))
        );
      ...
      if (handoverFrom || handoverTo) {
        ...
        if (propYear) {
          if (handoverFrom && propYear < parseInt(handoverFrom))
            matchHandover = false;
          if (handoverTo && propYear > parseInt(handoverTo))
            matchHandover = false;
        }
      }
    ...
    <div style={{ width: 320, padding: "12px 0 0", display: "flex", flexDirection: "column" }}>
      <div style={{ padding: "0 16px 12px" }}>
        ...
        <Input
          ...
          placeholder="Search..."
          value={devSearchText}
          onChange={(e) => setDevSearchText(e.target.value)}
      ...
      >
        {developers
          .filter((d) => d.name.toLowerCase().includes(devSearchText.toLowerCase()))
          .map((dev) => (
            <div
              key={dev._id || dev.id}
            ...
            >
              <Checkbox
                checked={selectedDevelopers.includes(dev._id || dev.id)}
                onChange={(e) => {
                  const id = dev._id || dev.id;
                  setSelectedDevelopers(
                    e.target.checked
                      ? [...selectedDevelopers, id]
                      : selectedDevelopers.filter((item) => item !== id)
                  );
                }}
              />
              <Avatar
                shape="square"
                src={dev.logo}
                style={{ margin: "0 12px", background: "#f3e8ff", color: "#5f0f9c" }}
                size="small"
              >
      ...
      <div style={{ display: "flex" }}>
        <Button
          type="text"
          style={{ flex: 1, borderRadius: 0, height: 44 }}
        ...
        <Button
          type="text"
          style={{ flex: 1, borderRadius: 0, height: 44 }}
          onClick={() => setDevPopoverOpen(false)}
        >
          Close
        </Button>
      </div>
  ...
  const pricePopoverContent = (
    <div style={{ width: 350, padding: "16px" }}>
      ...
      <Button
        type="primary"
        block
        style={{ marginTop: 16, height: 40, background: "#5f0f9c", border: "none" }}
    ...
    <div style={{ width: 320, padding: "16px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        <Text
          ...
          onClick={() => {
            setPreHandoverPct(100);
            setPostHandoverOnly(false);
          }}
        >
          Reset <CloseCircleOutlined />
        </Text>
      </div>
      <div style={{ marginBottom: 24 }}>
        <Slider
          ...
          onChange={setPreHandoverPct}
          trackStyle={{ background: "#5f0f9c" }}
          handleStyle={{ borderColor: "#5f0f9c" }}
        />
      </div>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginTop: 16 }}>
        ...
        <Text type="secondary" style={{ fontSize: 13, lineHeight: "1.2" }}>
          Search projects only with post handover payment plans
        </Text>
      </div>
    </div>
  );
        ...
        {unitTypeOptions.map((type) => {
          ...
          return (
            <div
              ...
              style={{
                padding: "6px 12px",
                background: isSelected ? "#5f0f9c" : "#f3f4f6",
  ...
  return (
    <div style={{ padding: "32px", background: "#f8f9fa", minHeight: "100vh" }}>
      ...
      <div style={{ marginBottom: 24, display: "flex", gap: "10px", flexWrap: "wrap", alignItems: "center" }}>
        ...
        <Popover
          content={devPopoverContent}
          trigger="click"
          open={devPopoverOpen}
          onOpenChange={setDevPopoverOpen}
          placement="bottomLeft"
          overlayInnerStyle={{ padding: 0 }}
        >
          <Button style={getFilterBtnStyle(selectedDevelopers.length > 0)}>
            Developer {selectedDevelopers.length > 0 && `(${selectedDevelopers.length})`}{" "}
        ...
        >
          <Button style={getFilterBtnStyle(preHandoverPct < 100 || postHandoverOnly)}>
            Payments {(preHandoverPct < 100 || postHandoverOnly) && "•"}{" "}
            <DownOutlined style={{ fontSize: 10 }} />
          </Button>
        </Popover>

        <Popover
          content={handoverPopoverContent}
          trigger="click"
          open={handoverPopoverOpen}
          onOpenChange={setHandoverPopoverOpen}
          placement="bottomLeft"
          overlayInnerStyle={{ borderRadius: 12 }}
        >
        ...
        <Popover
          content={unitTypePopoverContent}
          trigger="click"
          open={unitTypePopoverOpen}
          onOpenChange={setUnitTypePopoverOpen}
          placement="bottomLeft"
          overlayInnerStyle={{ borderRadius: 12 }}
        >
          <Button style={getFilterBtnStyle(selectedUnitTypes.length > 0)}>
            Unit type {selectedUnitTypes.length > 0 && `(${selectedUnitTypes.length})`}{" "}
            <DownOutlined style={{ fontSize: 10 }} />
          </Button>
        </Popover>
      </div>
                ...
                >
                  {p.developer?.logo ? (
                    ...
                    <span style={{ color: "#fff", fontSize: 16, fontWeight: "bold" }}>
                      {p.developerName ? p.developerName.charAt(0) : "D"}
                    </span>
                  )}
                </div>
              </div>
