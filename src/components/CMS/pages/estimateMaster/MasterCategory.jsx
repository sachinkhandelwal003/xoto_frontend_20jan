...
const MasterCategory = () => {
    ...
    async (page = 1) => {
      ...
      try {
        ...
        if (level === "subcategories") {
          ...
          const list = response?.data || response?.subcategories || [];
          setData(Array.isArray(list) ? list : []);
        } else if (level === "types") {
          url = `${API_BASE}/${parentCategory}/subcategories/${parentSubcategory}/types`;
          response = await apiService.get(url, params);

          const list = response?.data || response?.types || [];
          setData(Array.isArray(list) ? list : []);
        } else {
          // categories
          response = await apiService.get(url, params);
  ...
  // ✅ Reset page when navigating levels
  useEffect(() => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (level === "subcategories")
        url = `${API_BASE}/${parentCategory}/subcategories/${id}`;

      if (level === "types")
        url = `${API_BASE}/${parentCategory}/subcategories/${parentSubcategory}/types/${id}`;
  ...
  const columns = useMemo(() => {
    return [
        ...
        render: (_, record) => (
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            >
              ...
              {level === "subcategories" && (
                <FolderOpenOutlined
                  style={{ fontSize: "18px", color: THEME.secondary }}
                />
              )}
              {level === "types" && (
                <TagsOutlined style={{ fontSize: "18px", color: THEME.warning }} />
              )}
            </div>
      ...
      },
      {
        ...
        render: (_, record) => (
          <Tag
            color={record.isActive !== false ? "green" : "orange"}
  ...
  const EditModal = () => {
    ...
    useEffect(() => {
      if (selectedItem) {
        form.setFieldsValue({
          ...
          baseRatePerSqFt: selectedItem.baseRatePerSqFt ?? undefined,
        });
      }
    }, [selectedItem, form]);
    ...
    const onUpdate = async (values) => {
      ...
      try {
        let url = `${API_BASE}/${selectedItem._id}`; // categories
  ...
  return (
    <div style={{ padding: "24px", background: "#f5f7fa", minHeight: "100vh" }}>
      ...
      >
        <Space direction="vertical" size={4}>
          ...
          <Breadcrumb separator=">">
            ...
            {selectedSubcategory && (
              <Breadcrumb.Item>
                {capitalizeFirstLetter(selectedSubcategory.label)}
              </Breadcrumb.Item>
            )}
          </Breadcrumb>
        </Space>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: "24px" }}>
        {[
          { t: "Total", v: stats.total, c: THEME.primary, i: <AppstoreOutlined /> },
          { t: "Active", v: stats.active, c: THEME.success, i: <DatabaseOutlined /> },
          { t: "Trash/Inactive", v: stats.trashed, c: THEME.error, i: <RestOutlined /> },
        ].map((s, idx) => (
          <Col xs={24} sm={8} key={idx}>
            <Card
              bordered={false}
              style={{ borderRadius: "12px", borderBottom: `3px solid ${s.c}` }}
            >
      ...
      >
        ...
        {/* ✅ KEY prop -> level change pe table reset */}
        <CustomTable
      ...
      <EditModal key={`edit-${selectedItem?._id || "none"}`} />
      ...
      >
        {selectedItem && (
          <div style={{ padding: "8px 0" }}>
            <p>
              <Text type="secondary">ID:</Text> <br />
              <Text strong>{selectedItem._id}</Text>
            </p>

            <p>
              <Text type="secondary">Display Name:</Text> <br />
              <Text strong>
                {capitalizeFirstLetter(selectedItem.name || selectedItem.label)}
              </Text>
            </p>
            ...
            <p>
              ...
              <Text>
                {capitalizeFirstLetter(selectedItem.description) || "N/A"}
              </Text>
            </p>

            <p>
              <Text type="secondary">Current Level:</Text> <br />
              <Tag color="purple">{level.toUpperCase()}</Tag>
            </p>

            {(selectedItem.base_unit || selectedItem.baseRatePerSqFt) !== undefined && (
              <p>
                <Text type="secondary">Base Unit:</Text> <br />
                <Text strong>
                  {selectedItem.baseRatePerSqFt ?? selectedItem.base_unit}
                </Text>
              </p>
            )}
          </div>
