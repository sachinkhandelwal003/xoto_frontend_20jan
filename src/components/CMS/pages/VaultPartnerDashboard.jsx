...
const VaultPartnerDashboard = () => {
  ...
  // Determine if this is an individual partner
  const isIndividual = user?.partnerCategory === "individual" || partnerData?.partnerCategory === "individual";

  // Fetch partner profile using apiService
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const res = await apiService.get("/vault/partner/profile"); // adjust endpoint as needed
        const data = res?.data?.data || res?.data || {};
        setPartnerData(data);
      } catch (error) {
        console.error("Failed to fetch partner profile:", error);
        // Fallback: use user from Redux if available
  ...
  const partnerName = partnerData?.companyName || partnerData?.individualDetails?.firstName
    ? partnerData?.companyName ||
      `${partnerData.individualDetails?.firstName || ""} ${partnerData.individualDetails?.lastName || ""}`.trim()
    : user?.name || "Partner";

  // 📈 Mock revenue data – replace with real API data when available
  const revenueData = [
    ...
    <div style={{ background: "#f9f6ff", minHeight: "100vh", padding: "32px 28px" }}>
      ...
      <div className="vpp">
          ...
          <Space size="middle">
            {isIndividual && (
              <Button
                ...
                size="large"
                icon={<PlusOutlined />}
                onClick={() => navigate("/dashboard/vaultpartner/leads/create")}
                style={{
        ...
        <Row gutter={[20, 20]} style={{ marginBottom: 28 }}>
          <Col xs={24} lg={15}>
            <Card bordered={false} className="vpp-chart-card" title={<span style={{ fontSize: 16, fontWeight: 700, color: P }}>Revenue Milestone (AED)</span>}>
          ...
          <Col xs={24} lg={9}>
            <Card bordered={false} className="vpp-chart-card" title={<span style={{ fontSize: 16, fontWeight: 700, color: P }}>Project Sales Distribution</span>}>
              <ResponsiveContainer width="100%" height={320}>
        ...
        <Row>
          <Col xs={24}>
            <Card bordered={false} className="vpp-chart-card" title={<span style={{ fontSize: 16, fontWeight: 700, color: P }}>Recent Partner Activities</span>}>
              <List
