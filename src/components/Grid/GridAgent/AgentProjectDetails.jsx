...
const fetchInventoryUnits = async (propertyId) => {
  try {
    ...
    // ✅ Correct route — uses query param, not path param
    ...
    {["all", "available", "reserved", "booked", "sold"].map(tab => (
      <button
        ...
        style={{
          ...
          cursor: "pointer",
          fontSize: 13,
          fontWeight: activeUnitTab === tab ? 700 : 400,
          background: activeUnitTab === tab ? "#111827" : "#fff",
          color: activeUnitTab === tab ? "#fff" : "#6b7280",
          borderColor: activeUnitTab === tab ? "#111827" : "#e5e7eb",
        }}
      >
        ...
        {tab !== "all" && inventoryCounts[tab] > 0 && (
          <span style={{
            marginLeft: 6,
            background: activeUnitTab === tab ? "rgba(255,255,255,0.2)" : "#f3f4f6",
            borderRadius: 10,
            padding: "1px 6px",
            fontSize: 11,
          }}>
