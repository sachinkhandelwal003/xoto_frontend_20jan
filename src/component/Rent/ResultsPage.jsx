...
function LeadModal({ property, onClose, onSubmit, submitting }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",backdropFilter:"blur(4px)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:16 }}>
      <div onClick={(e)=>e.stopPropagation()} style={{ background:"white",borderRadius:24,width:"100%",maxWidth:520,boxShadow:"0 32px 80px rgba(109,40,217,0.2)",overflow:"hidden" }}>
        <div style={{ background:"#5c039c",padding:"24px 28px",position:"relative" }}>
          <button onClick={onClose} style={{ position:"absolute",top:16,right:16,background:"rgba(255,255,255,0.15)",border:"none",borderRadius:8,color:"white",width:32,height:32,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center" }}><FiX size={16} /></button>
          ...
          <div style={{ fontSize:13,color:"rgba(255,255,255,0.7)",marginTop:4,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap" }}>
            <span style={{ display:"flex",alignItems:"center",gap:6 }}><FiMapPin size={14} />{property?.location?.area&&`${property.location.area}, `}{property?.emirate}</span>
            {property?.price&&<span style={{ background:"rgba(255,255,255,0.15)",padding:"2px 10px",borderRadius:20,fontWeight:700,color:"white" }}>AED {property.price.toLocaleString()}/yr</span>}
          </div>
        </div>
        <form onSubmit={handleSubmit} style={{ padding:"24px 28px" }}>
          <p style={{ fontSize:13,color:"#64748b",marginBottom:20,lineHeight:1.6 }}>Fill in your details and our agent will contact you shortly.</p>
          <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
            ...
            <div>
              ...
              <textarea name="message" value={form.message} onChange={handleChange} placeholder="Any specific requirements..." rows={3}
          ...
          <button type="submit" disabled={submitting}
            ...
            {submitting ? <><span style={{ display:"inline-block",width:18,height:18,border:"2px solid rgba(255,255,255,0.3)",borderTopColor:"white",borderRadius:"50%",animation:"spin 0.8s linear infinite" }} /> Sending...</> : <><FiPhone size={16} /> Send Interest</>}
          </button>
        </form>
      </div>
    </div>
...
// ─── LISTING CARD ─────────────────────────────────────────────────────────────
function ListingCard({ listing, saved, onSave, onContact, leadCreating, contacted, isMobile }) {
        ...
        <div style={{ display:"flex",flexDirection:isMobile?"column":"row",gap:0 }}>
          ...
          <div style={{ width:isMobile?"100%":320,flexShrink:0,padding:12,position:"relative",boxSizing:"border-box" }}>
            ...
            <div style={{ position:"absolute",top:20,left:20,display:"flex",flexDirection:"column",gap:6 }}>
              {listing.verified&&<div style={{ background:"#6d28d9",color:"#fff",fontSize:10,fontWeight:700,borderRadius:6,padding:"4px 10px",textTransform:"uppercase",display:"flex",alignItems:"center",gap:4 }}><FiCheck size={10} /> Verified</div>}
              {listing.ejari&&<div style={{ background:"rgba(0,0,0,0.72)",backdropFilter:"blur(6px)",color:"#fff",fontSize:10,fontWeight:700,borderRadius:6,padding:"4px 10px",display:"flex",alignItems:"center",gap:4 }}><FiFileText size={10} /> Ejari</div>}
            </div>
            <button onClick={()=>onSave(listing._id)} style={{ position:"absolute",top:20,right:20,background:saved?"#6d28d9":"rgba(255,255,255,0.9)",border:"1.5px solid "+(saved?"#6d28d9":"rgba(0,0,0,0.1)"),borderRadius:10,width:36,height:36,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s",color:saved?"#fff":"#94a3b8" }}>
          ...
          <div style={{ flex:1,padding:isMobile?"0 16px 16px":"20px 24px 16px 12px",display:"flex",flexDirection:"column",gap:12,minWidth:0 }}>
            <div style={{ display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8,flexWrap:"wrap" }}>
              ...
              <div style={{ textAlign:"right",flexShrink:0 }}>
                <div style={{ fontSize:isMobile?17:20,fontWeight:900,color:"#6d28d9",letterSpacing:"-0.5px" }}>AED {(listing.monthly||Math.round(listing.price/12)).toLocaleString()}</div>
            ...
            <div style={{ display:"flex",gap:8,flexWrap:"wrap",alignItems:"center" }}>
              ...
              {listing.tenants&&<span style={{ background:"#f1f5f9",color:"#475569",borderRadius:6,fontSize:11,fontWeight:600,padding:"3px 10px",display:"flex",alignItems:"center",gap:4 }}><FiUser size={12} />{listing.tenants}</span>}
            </div>
  ...
  return (
    <div>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20 }}>
        ...
        <span onClick={onReset} style={{ fontSize:12,fontWeight:700,color:"#ef4444",cursor:"pointer" }}>Reset all</span>
      </div>
...
export default function ResultsPage() {
  ...
  return (
    <div style={{ background:"#f7f6fb",minHeight:"100vh",fontFamily:"system-ui, sans-serif" }}>
      ...
      {leadModal && <LeadModal property={leadModal} onClose={()=>setLeadModal(null)} onSubmit={handleLeadSubmit} submitting={leadSubmitting} />}

      {/* Mobile backdrop */}
      {isMobile && filterOpen && (
        <div onClick={()=>setFilterOpen(false)} style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:998 }} />
      )}

      {/* Mobile filter drawer */}
      {isMobile && (
        <div id="filter-drawer" style={{ position:"fixed",top:0,left:0,bottom:0,width:"85vw",maxWidth:340,background:"white",zIndex:999,padding:20,overflowY:"auto",transform:filterOpen?"translateX(0)":"translateX(-100%)",opacity:filterOpen?1:0,boxShadow:filterOpen?"4px 0 24px rgba(0,0,0,0.15)":"none",boxSizing:"border-box" }}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16 }}>
            <span style={{ fontSize:16,fontWeight:900,color:"#1e1b4b" }}>Filters</span>
            <button onClick={()=>setFilterOpen(false)} style={{ background:"#f1f5f9",border:"none",borderRadius:8,width:32,height:32,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#475569" }}><FiX size={16} /></button>
          </div>
          <FilterContent {...filterProps} />
        </div>
      )}

      <div style={{ display:"grid",gridTemplateColumns:isMobile?"1fr":"264px 1fr 280px",gap:20,maxWidth:1400,margin:"0 auto",padding:isMobile?"16px":"24px",boxSizing:"border-box" }}>
        ...
        <div>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:16,flexWrap:"wrap",gap:10 }}>
            <div>
              ...
              <div style={{ fontSize:isMobile?18:22,fontWeight:900,color:"#1e1b4b",letterSpacing:"-0.5px" }}>{loading?"Loading...":`${total} Properties found`}</div>
            </div>
          ...
          {loading ? (
            ...
            <div style={{ background:"white",borderRadius:20,padding:"80px 20px",textAlign:"center",color:"#94a3b8",border:"1.5px dashed #e2e8f0" }}>
              <div style={{ marginBottom:16 }}><FiHome size={40} /></div>
          ...
          {isMobile && (
            <div style={{ marginTop:24,display:"flex",flexDirection:"column",gap:20 }}>
              ...
              <div style={{ background:"linear-gradient(145deg,#1e1b4b 0%,#4c1d95 100%)",borderRadius:16,padding:"22px 20px",color:"white",textAlign:"center",boxShadow:"0 8px 28px rgba(109,40,217,0.22)" }}>
                <div style={{ marginBottom:10 }}><FiFileText size={28} /></div>
